  ...
  return (
    <div className="card">
      ...
      <div className="flex justify-between">
        {MOODS.map((m) => (
          <button key={m.value} onClick={() => log(m.value)} className="flex h-12 w-12 items-center justify-center rounded-2xl bg-sand text-2xl transition active:scale-90">
            {m.icon}
          </button>
        ))}
      </div>
    ...
    <div className="space-y-4">
      ...
      <div className="card">
        ...
        <button onClick={() => setSheet('appointment')} className="btn-outline w-full">
          📅 Add appointment & questions for provider
        </button>
      </div>
